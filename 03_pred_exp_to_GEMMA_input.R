...
for(tiss in tiss_list){
  pred_exp <- fread("/home/angela/px_his_chol/MESAreplication/PrediXcan/" %&% tiss %&% "_predicted_expression.txt", header = F) #path to PrediXcan-produced _predicted_expression.txt
  pred_exp$V1 <- NULL
  pred_exp$V2 <- NULL
  ...
  pred_exp <- cbind(gene_list, pred_exp)
  fwrite(pred_exp, "/home/angela/px_his_chol/MESAreplication/GEMMA/pred_exp/" %&% tiss, na = "NA", sep = "\t", col.names = F) #path to GEMMA input/pseudo-genotype
}
