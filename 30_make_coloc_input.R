...
for(pop in length(pops)){ #read in pop's .frq file for MAF
  ...
  for(pheno in phenos){ #read in GEMMA output file
    ...
    for(chr in chrs){ #yes triple loops are ratchet
      ...
      fwrite(meQTL_for_COLOC_chr, "/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pops[pop] %&% "_for_COLOC_chr" %&% chr %&% "_" %&% pheno %&% ".txt", quote = F, sep = "\t", na = "NA", row.names = F, col.names = T)
      gzip("/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pops[pop] %&% "_for_COLOC_chr" %&% chr %&% "_" %&% pheno %&% ".txt", destname = "/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pops[pop] %&% "_for_COLOC_chr" %&% chr %&% "_" %&% pheno %&% ".txt.gz") #script may only take .gz values so can't hurt to be too careful
      fwrite(GEMMA_for_COLOC_chr, "/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pheno %&% "_chr" %&% chr %&% "_for_COLOC.txt", row.names = F, col.names = T, sep = "\t", quote = F, na = "NA")
      gzip("/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pheno %&% "_chr" %&% chr %&% "_for_COLOC.txt", "/home/angela/px_his_chol/COLOC/COLOC_input/" %&% pheno %&% "_chr" %&% chr %&% "_for_COLOC.txt.gz")
