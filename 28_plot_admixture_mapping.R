...
plot_admixture_mapping <- function(chr, pheno, tissue, gene){
  ...
  admixture_plot <- ggplot() + 
    geom_point(data = GWAS, aes(x = ps, y = -log10(p_wald), fill = predictor), shape = 21, stroke = 0, size = 1.5, color = "transparent") +
    ...
    scale_x_continuous(label = function(x) {ifelse(x == 0, "0", parse(text = gsub("[+]", "", gsub("e", " %*% 10^", scientific_10(GWAS$ps)(GWAS$ps)))))}) + #https://stackoverflow.com/questions/10762287/how-can-i-format-axis-labels-with-exponents-with-ggplot2-and-scales
    xlab("bp") + 
    ylab("-log10(p)") +
    theme_bw() +
    ...
    scale_fill_grey() #boring but it works
