...
sub_pop = ["YRI", "PEL", "IBS"]
#tbh I could've prob made a dict

#anchor_pops = open(open_dir + "px_his_chol/ADMIXTURE/anchor_pops.txt", "w")
for select_pop in range(0, len(super_pop)):
    anchor_pops = open(open_dir + "px_his_chol/ADMIXTURE/" + sub_pop[select_pop] + "_fam.txt", "w")
    ...
    #os.system("plink --bfile " + open_dir + "1000G/1000G2 --keep " + open_dir + "px_his_chol/ADMIXTURE/" + sub_pop[select_pop] + "_fam.txt --make-bed --out " + open_dir + "px_his_chol/ADMIXTURE/" + sub_pop[select_pop])
