...
HCHS_yGTEx <- fread('/home/angela/px_his_chol/PrediXcan/TW_Esophagus_Muscularis_0.5.db_predicted_expression.txt', header = T)
...
HCHS <- ggplot(HCHS_xyGTEx, aes(x = HCHS_xyGTEx$ENSG00000090339.4, y = HDL_rank)) + 
...
MESA_HIS_yGTEx <- fread('/home/angela/px_his_chol/MESAreplication/PrediXcan/TW_Esophagus_Muscularis_0.5.db_predicted_expression.txt')
...
MESA_HIS <- ggplot(MESA_HIS_xyGTEx, aes(x = MESA_HIS_xyGTEx$ENSG00000090339.4, y = HDL_rank)) + 
  geom_jitter(size = 0.75, color = "#b3cde3") + 
  geom_density_2d(color = "#8c96c6") + 
  stat_smooth(method="lm", se = TRUE, fullrange = TRUE, color = "#8856a7") + 
  scale_x_continuous(name = "Predicted gene expression") + 
...
pdf("/home/angela/px_his_chol/Manuscript_figures/Fig5.pdf", width = 5.5, height = 3)
plot_grid(HCHS, MESA_HIS, labels = c('A', 'B'), scale = c(.9, .9, .9, .9))
dev.off()

tiff("/home/angela/px_his_chol/Manuscript_figures/Fig5.tiff", width = 13.97, height = 7.62, units = 'cm', res = 300, compression = 'lzw')
plot_grid(HCHS, MESA_HIS, labels = c('A', 'B'), scale = c(.9, .9, .9, .9))
dev.off()